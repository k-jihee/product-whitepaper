...
<div id="print-area">
    ...
    {f"<h3>9. 한도견본</h3>" + ''.join(f'<img src=\"{link.strip()}\" width=\"300\">' for link in str(row.get("한도견본", "")).split(",") if link.strip()) if pd.notna(row.get("한도견본", "")) else ""}" width="300">' for link in str(row.get("한도견본", "")).split(",") if link.strip())}
<br>
