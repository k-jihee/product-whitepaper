...
st.markdown("---")
st.markdown('<h4>🆚 <b>여러 제품을 비교하려면 쉼표로 구분해서 입력하세요</b></h4>', unsafe_allow_html=True)
compare_query = st.text_input("예: GIB1010, GIB2010", key="compare_input")

if compare_query:
    compare_terms = [term.strip() for term in compare_query.split(",") if term.strip()]
    compare_results = df[df["제품코드"].astype(str).isin(compare_terms) | df["제품명"].astype(str).isin(compare_terms)]

    if not compare_results.empty:
        cols = st.columns(len(compare_results))
        for col, (_, row) in zip(cols, compare_results.iterrows()):
            with col:
                st.markdown(f"### 📦 {row.get('제품명', '-')}")
                st.markdown(f"**제품코드:** {row.get('제품코드', '-')}")
                st.markdown(f"**식품유형:** {row.get('식품유형', '-')}")
                st.markdown(f"**구분:** {row.get('구분', '-')}")
                st.markdown(f"**소비기한:** {row.get('소비기한', '-')}")
                st.markdown(f"**용도:** {row.get('용도', '-')}")
                st.markdown(f"**주요거래처:** {row.get('주요거래처', '-')}")
                st.markdown(f"**2022 생산:** {clean_int(row.get('생산실적(2022)'))}")
                st.markdown(f"**2023 생산:** {clean_int(row.get('생산실적(2023)'))}")
                st.markdown(f"**2024 생산:** {clean_int(row.get('생산실적(2024)'))}")
                st.markdown("**제품특징:**")
                st.markdown(format_features(row.get('제품특징', '-')), unsafe_allow_html=True)
    else:
        st.warning("❌ 비교할 수 있는 제품이 없습니다. 제품코드 또는 제품명을 확인하세요.")
