...
# 검색창 문구 및 크기 확대
st.markdown("### 🔎 제품을 검색하세요 (제품코드 또는 제품명)")
query = st.text_input("예: P001 또는 고구마칩", key="query_input")

# PDF용 한글 폰트 포함 FPDF 클래스
class PDF(FPDF):
    def __init__(self):
        super().__init__()
        self.add_page()
...
def generate_pdf(data):
    pdf = PDF()
    pdf.cell(200, 10, txt="제품 백서 - " + str(data.get("제품명", "-")), ln=True, align="C")
    pdf.ln(10)

    fields = [
        "용도", "식품유형", "구분", "제품코드", "소비기한",
    ...
    for key in fields:
        text = str(data.get(key, "-")).replace("\n", "
").replace("", "")
        lines = text.splitlines()
        if not lines:
            lines = ["-"]
        pdf.multi_cell(0, 10, f"{key}: {lines[0]}")
        for line in lines[1:]:
