...
if pd.notna(img_src):
    ...
    for i, link in enumerate(str(img_src).split(","), start=1):
        link = link.strip()
        if link.startswith("http") or os.path.exists(link):
            html = f"""
<a href='{link}' target='_blank' download>
    <img src='{link}' width='300' style='margin: 10px; border: 1px solid #ccc; border-radius: 8px;' alt='한도견본 {i}'>
</a>
<div style='font-size: 13px; text-align: center;'>🖱️ 클릭하면 새 탭에서 열리고 자동 다운로드됩니다.</div>
"""
