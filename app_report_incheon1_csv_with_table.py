...
# ============================
def page_chatbot():
    # 0) 이 페이지에서는 헤더/여백/스크롤 없애고 전체 화면 쓰기
    st.markdown(
        """
        <style>
        /* 상단 기본 헤더 숨기기 */
    ...
    # 1) 화면 전체를 챗봇 iframe으로 덮기 (이 화면만 보이도록)
    iframe_html = """
    <iframe
    ...
    # height/width는 의미 없고, 실제 크기는 위 style로 결정됨
    st.components.v1.html(iframe_html, height=0, width=0)
