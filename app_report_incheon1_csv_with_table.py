...
'''
            ...
            html_content = html_template.format(원재료_원산지=row.get('원재료명 및 함량', '-') + ' / 원산지: ' + row.get('원산지', '-'), 
                제품명=row.get('제품명', '-'),
                용도=row.get('용도', '-'),
                식품유형=row.get('식품유형', '-'),
                구분=row.get('구분', '-'),
                ...
                주요거래처=row.get('주요거래처', '-'),
                제조방법=row.get('제조방법', '-'),
                원재료=row.get('원재료명 및 함량', '-'),
                원산지=row.get('원산지', '-'),
                제품특징=row.get('제품특징', '-'),
                기타사항=row.get('기타사항', '-'),
                성상_row=성상_row,
                spec_rows=성상_row + spec_rows
            )
